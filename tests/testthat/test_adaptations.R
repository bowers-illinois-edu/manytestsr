...
## Since this does not descend the tree, now test using some non-null effect blocks.
...
setkey(res_half, "bF")
setkey(res_half_hommel, "bF")
setkey(res_half_unadj, "bF")
setkey(res_half_simes, "bF")
setkey(res_half_bh, "bF")
setkey(res_half_minp, "bF")

expect_equal(res_half$pfinalb, res_half_unadj$pfinalb)
...
all.equal(res_half_tree$node_number, res_half_hommel_tree$node_number)
all.equal(res_half_tree$node_number, res_half_simes_tree$node_number)
all.equal(res_half_tree$node_number, res_half_bh_tree$node_number)
all.equal(res_half_tree$node_number, res_half_minp_tree$node_number)

all.equal(res_half_tree$depth, res_half_hommel_tree$depth)
all.equal(res_half_tree$depth, res_half_simes_tree$depth)
all.equal(res_half_tree$depth, res_half_bh_tree$depth)
all.equal(res_half_tree$depth, res_half_minp_tree$depth)

res_comp <- merge(res_half_tree[, .(name, node_number, depth, p)],
  res_half_hommel_tree[, .(name, node_number, p)],
  by = "node_number"
) %>%
  arrange(node_number) %>%
  as.data.table()
...
cbind(
  res_half_tree[, .(name, depth, p)],
  res_half_hommel_tree[, .(name, depth, p)],
  res_half_tree$p - res_half_hommel_tree$p
)

cbind(
  res_half[, .(bF, pfinalb, biggrp)],
  res_half_hommel[, .(bF, pfinalb, biggrp)],
  res_half$pfinalb - res_half_hommel$pfinalb
)
