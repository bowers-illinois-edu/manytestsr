...
get_children_nodes <- function(parent_id, node_tracker) {
  if (is.null(node_tracker$tracker)) {
    return(integer(0))
  }
...
generate_meinshausen_hierarchy <- function(correlation_matrix, method = "complete",
  ...
  for (i in seq_len(nrow(hc$merge))) {
    ...
    if (left_child < 0) {
      left_child <- abs(left_child)
    } else {
      left_child <- left_child + n_vars
    }

    if (right_child < 0) {
      right_child <- abs(right_child)
    } else {
      right_child <- right_child + n_vars
    }
...
integrate_meinshausen_find_blocks <- function(node_dat, node_tracker, alpha = 0.05,
  ...
  tryCatch(
    ...
    error = function(e) {
      warning(
        "Meinshausen hierarchical testing failed: ", e$message,
